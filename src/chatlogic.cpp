...
ChatLogic::~ChatLogic() {
  ...
  // delete all nodes (modified)
  // for (auto it = std::begin(_nodes); it != std::end(_nodes); ++it) {
  //   delete *it;
  // }

  // delete all edges (modified)
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename) {
      ...
      if (type != tokens.end()) {
        ...
        if (idToken != tokens.end()) {
          ...
          if (type->second == "NODE") {
            ...
            // check if node with this ID exists already (modified)
            auto newNode =
            ...
            // create new element if ID does not yet exist (modified)
          ...
          if (type->second == "EDGE") {
            ...
            if (parentToken != tokens.end() && childToken != tokens.end()) {
              ...
              // create new edge (modified to unique_ptr)
              std::unique_ptr<GraphEdge> edge = std::make_unique<GraphEdge>(id);
              GraphEdge *tempEdge = edge.get();
              // GraphEdge *edge = new GraphEdge(id);
              edge->SetChildNode(childNode->get());
              edge->SetParentNode(parentNode->get());
              _edges.push_back(tempEdge);

              // find all keywords for current node
              AddAllTokensToElement("KEYWORD", tokens, *edge);

              // store reference in child node and parent node
              (*childNode)->AddEdgeToParentNode(tempEdge);
  ...
  for (auto it = std::begin(_nodes); it != std::end(_nodes); ++it) {
    ...
    if ((*it)->GetNumberOfParents() == 0) {
      if (rootNode == nullptr) {
        rootNode = it->get(); // assign current node to root (modified)
      } else {
  ...
  // create new instance of chatbot
  ChatBot new_chatBot = ChatBot("../images/chatbot.png");
